<#
.SYNOPSIS
    Comprehensive Active Directory Audit Script with Enhanced Reporting
.DESCRIPTION
    Performs detailed security and health checks on Active Directory with color-coded output
    and exports to CSV/HTML formats
.NOTES
    Version: 2.0
...
# Computer Accounts
try {
    $computerStats = Get-ADComputer -Filter * -Properties LastLogonDate | Group-Object -Property { $_.LastLogonDate -lt (Get-Date).AddDays(-90) } -AsHashTable
...
$csvPath = "AD_Comprehensive_Audit_$timestamp.csv"
$htmlPath = "AD_Comprehensive_Audit_$timestamp.html"
